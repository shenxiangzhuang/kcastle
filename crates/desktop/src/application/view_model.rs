...
pub(crate) fn conversation_view_model(state: &AppState) -> ConversationViewModel<'_> {
    ConversationViewModel {
        empty: state.session_view.conversation.messages.is_empty()
            && state.transient_messages.is_empty(),
        title: &state.session_view.conversation.title,
        turns: state.session_view.conversation.turns,
        steps: step_count(state.session_view.conversation.messages.iter()),
    }
}
...
pub(crate) fn composer_status(state: &AppState) -> String {
    ...
    if stats.steps > 0 {
        ...
        if stats.tool_ns > 0 {
            durations.push(format!("Tool call {}", compact_duration(stats.tool_ns)));
        }
        ...
        if stats.decode_ns > 0 {
            speeds.push(format!(
                "{} tok/s",
                compact_rate(
                    stats.decode_tokens as f64 / (stats.decode_ns as f64 / 1_000_000_000.0),
                )
            ));
        }
        if !speeds.is_empty() {
...
pub(crate) fn step_count<'a, T: Borrow<Message> + 'a>(
    messages: impl IntoIterator<Item = &'a T>,
) -> usize {
    messages
        .into_iter()
        .map(Borrow::borrow)
...
fn compact_number(value: u64) -> String {
    if value >= 1_000_000 {
        format_compact_decimal(value as f64 / 1_000_000.0, "M")
    } else if value >= 1_000 {
        format_compact_decimal(value as f64 / 1_000.0, "K")
    } else {
        value.to_string()
    }
}
...
fn format_compact_decimal(value: f64, suffix: &str) -> String {
    let rendered = if value >= 100.0 {
        ((value + 0.5).floor() as u64).to_string()
    } else {
        format!("{value:.1}")
    };
    format!(
...
mod tests {
    ...
    fn empty_state_projects_to_stable_view_data() {
        ...
        assert_eq!(composer_status(&state), "");
    }
    ...
    fn token_counts_are_compact() {
        ...
        assert_eq!(compact_number(13_000), "13K");
        assert_eq!(compact_number(516_500), "517K");
