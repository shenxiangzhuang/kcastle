...
impl FontLoadPlan {
    pub fn for_display_items(items: &[DisplayItem]) -> Self {
            ...
            } = item
            {
                if let Some(font_id) = FontId::parse(font) {
                    ...
                    if may_need_runtime_unicode_fallback(font_id, *char_code) {
                        fallback_chars.push(ratex_font::katex_ttf_glyph_char(font_id, *char_code));
                    }
                }
            }
        }
        ...
        if !fallback_chars.is_empty() {
            ...
            // A Unicode glyph need not load the entire system emoji font. Keep the
            // existing fallback chain only when the primary face cannot draw it.
...
mod tests {
    ...
    #[test]
    fn missing_unicode_glyph_keeps_optional_fallbacks() {
        let plan =
            FontLoadPlan::for_display_items(&[glyph(FontId::MainRegular, '\u{10ffff}' as u32)]);
    ...
    #[test]
    fn missing_explicit_cjk_glyph_keeps_required_font_and_fallbacks() {
        let plan =
            FontLoadPlan::for_display_items(&[glyph(FontId::CjkRegular, '\u{10ffff}' as u32)]);
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn covered_unicode_does_not_request_emoji_or_second_font() {
        // This regression uses the same system font as the macOS memory reproduction.
        if std::env::var_os("RATEX_UNICODE_FONT").is_some()
            || !std::path::Path::new("/Library/Fonts/Arial Unicode.ttf").exists()
        {
            return;
        }
        for (font, ch) in [(FontId::CjkRegular, '中'), (FontId::MainRegular, '⌘')] {
            let plan = FontLoadPlan::for_display_items(&[glyph(font, ch as u32)]);
            assert!(!plan.optional.contains(&FontId::EmojiFallback));
            assert!(!plan.optional.contains(&FontId::CjkFallback));
        }
    }
